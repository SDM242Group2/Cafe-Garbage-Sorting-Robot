...
float speed[4];
//char array
uint8_t usart3_recv[15];
//int array for speed x, y, turn.
int16_t spd[3] = {0, 0, 0};
int16_t receiveLedFlag = 0;
int16_t transmitLedFlag = 0;
int16_t transmitCount = 0;
...
void Usart3_callback(void){
	...
	// process raw string data to spd
	for (int i = 0; i < 3; i++) {
		for (int j = 4; j >= 0; j--) {
			int p = (5 * i + j);
			switch(j) {
				case 4: // space
					break;
				case 3: // from last digit
					spd[i] += (usart3_recv[p] - 48);
					break;
				case 2:
					spd[i] += ((usart3_recv[p] - 48) * 10);
					break;
				case 1:
					spd[i] += ((usart3_recv[p] - 48) * 100);
					break;
				case 0: // sign, 0 for positive, 1 for negative.
					if (usart3_recv[p] - 48 == 1) {
						spd[i] *= -1;
					}
					break;
			}
...
void study_task(const void*argu){
	...
	uart_recv_callback_register(USER_UART3,Usart3_callback);
	//从uart3接收，把接收到的数据放到usart3_recv
	uart_receive_start(USER_UART3,usart3_recv,15);
	...
	while(1){
		//当遥控器左上角的拨档开关拨到上, or initial value. Auto mode
		if(rc.sw1==1 || rc.sw1==0){
			speed_x = spd[0]*4/4;
			speed_y = spd[1]*4/4;
			speed_turn = spd[2]*4/4;
			
			if (transmitCount == 1) { // transmit rate, 1 sometimes works well
		...
		} else { // rc.sw1 == 3 or 2. manual mode.
			speed_x = rc.ch2*4/4;
			speed_y = rc.ch1*4/4;
		...
		// pid calulation & current calculation
		current[0] = pid_calc(&pid_wheel_spd[0], moto_chassis[0].speed_rpm, speed[0]);
		current[1] = pid_calc(&pid_wheel_spd[1], moto_chassis[1].speed_rpm, speed[1]);
		current[2] = pid_calc(&pid_wheel_spd[2], moto_chassis[2].speed_rpm, speed[2]);
		current[3] = pid_calc(&pid_wheel_spd[3], moto_chassis[3].speed_rpm, speed[3]);
		
		// for satety, stop the current if current excede +-7000.
		for (int i = 0; i < 4; i++) {
			if (current[i] > 7000 || current[i] < -7000) {
				current[i] = 0;
			}
		}
